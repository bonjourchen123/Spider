...
<script>
	...
	function tetGrid(){
		$("#resultTable").jqGrid('setGridParam', {url: "<s:url namespace='/policy' action='queryVerify'/>"+"?"+$("#queryForm").serialize(),datatype: 'json'});
		$("#resultTable").jqGrid().trigger('reloadGrid');
	}
	
	$(document).ready(function(){
		var gridOption = {
 			url:'<s:url namespace="/policy" action="verify"/>',
			datatype: 'json',
			mtype: 'GET',
	        ...
	        colModel:[
	            ...
	            {name:'assessorDate',index:'assessorDate', width:110, align:"center",
	            	formatter:"date",formatoptions:{newformat:"Y-m-d"},formatter:function(cellvalue, options, rowObject){
	            		if(cellvalue==null){
	            			return "未審核";
						}else{
	            			return cellvalue;
		...
		gridOption.gridComplete = function() {
			...
			for (var i=0; i<ids.length; i++) {
				...
				var se = '<input type="button" onclick="verift('+id+')" value="審核">';
				var func = se;
	...
	function verift(id){
		var form = document.createElement("form");
 	    ...
 	    var path='<c:url value="/policy/showVerify"/>';
...
<table class="myTable">
	<s:form method="get" namespace="/policy" action="queryVerify" theme="simple" id="queryForm">
		<tr>
			<td>			
